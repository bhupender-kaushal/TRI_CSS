...
def ACELoss(y_pred, y_true, u=1, a=1, b=1):
    ...
    def region(y_pred, y_true, u=1):
        ...
        region_in = torch.sum(torch.sum(y_pred * ((label - c_in) ** 2))) #sum
        region_out = torch.abs(
            torch.sum((1 - y_pred) * ((label - c_out) ** 2))) #sum
...
def weighted_binary_cross_entropy_loss(y_actual, target, weight=1):

    input = torch.clamp(y_actual,min=1e-7,max=1-1e-7)
